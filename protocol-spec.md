...
| `CAPABILITIES` | both | `sample_rate` (Hz), `channels`, `stream_mode` (optional) | Exchanged after pairing so both sides agree on format before audio starts. In microphone mode, `channels` is typically `1` (mono) and `sample_rate` is `48000`. In duplex mode, both speaker playback and mic capture channels are negotiated and run simultaneously. |
