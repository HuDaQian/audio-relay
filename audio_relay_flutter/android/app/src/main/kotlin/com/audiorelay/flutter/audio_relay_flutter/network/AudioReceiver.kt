...
import com.audiorelay.flutter.audio_relay_flutter.audio.AudioLevel
import com.audiorelay.flutter.audio_relay_flutter.audio.JitterBuffer
import com.audiorelay.flutter.audio_relay_flutter.audio.PlaybackTrack
...
class AudioReceiver {
    ...
    /** Runs until [close] is called, reading length-prefixed frames from a TCP stream (for ADB reverse mode). */
    suspend fun receiveTcpLoop(stream: java.io.InputStream) = withContext(Dispatchers.IO) {
        val buf = ByteArray(MAX_PACKET_SIZE)
        ...
        while (isActive && !closed) {
            try {
                var read = 0
                while (read < 2) {
                    val r = stream.read(lenBuf, read, 2 - read)
                    if (r < 0) return@withContext
                    read += r
                }
                val packetLen = ((lenBuf[0].toInt() and 0xFF) shl 8) or (lenBuf[1].toInt() and 0xFF)
                if (packetLen <= 0 || packetLen > MAX_PACKET_SIZE) continue

                read = 0
                while (read < packetLen) {
                    val r = stream.read(buf, read, packetLen - read)
                    if (r < 0) return@withContext
                    read += r
                }

                val header = AudioPacket.decodeHeader(buf, packetLen) ?: continue
                ...
                val pcm = Crypto.decryptPayload(
                    ...
                    ciphertext = buf,
                    ciphertextOffset = header.payloadStart,
                    ciphertextLength = packetLen - header.payloadStart,
                )
                jitterBuffer?.push(header.sequence, pcm, header.timestampMs)
            } catch (e: Exception) {
